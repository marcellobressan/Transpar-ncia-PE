  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => setView(ViewState.LIST)}>
            ...
            <div>
              ...
              <p className="text-xs text-brand-300">Auditoria Cívica de Candidatos</p>
            </div>
          </div>
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8">
              <div className="max-w-3xl">
                <h2 className="text-2xl font-bold text-gray-900 mb-2">Quem custa mais para Pernambuco?</h2>
                <p className="text-gray-600 mb-6">
                  Analisamos o histórico de gastos, emendas e eficiência de candidatos federais, estaduais e municipais. 
                  Tome sua decisão baseada em dados, não apenas em promessas.
                </p>
            ...
            <div>
              <div className="flex items-center justify-between mb-4">
                 ...
                 <span className="text-sm text-gray-500">{filteredCandidates.length} candidatos encontrados</span>
              </div>
              
              {filteredCandidates.length > 0 ? (
                <div className="bg-white rounded-lg shadow overflow-hidden">
                  <table className="min-w-full divide-y divide-gray-200">
                     {/* Mobile friendly list is handled via ComparisonTable responsive styling */}
                     <ComparisonTable candidates={filteredCandidates} />
                  </table>
                  <div className="p-4 bg-gray-50 border-t border-gray-200 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                     {filteredCandidates.map(c => (
                        <button 
                          ...
                          className="flex items-center gap-4 p-3 bg-white border border-gray-200 rounded-lg hover:shadow-md transition text-left group"
                        >
                          <img src={c.photoUrl} className="w-12 h-12 rounded-full object-cover group-hover:opacity-80" alt="" />
                          <div>
                            <p className="font-bold text-gray-900 group-hover:text-brand-600">{c.name}</p>
                            <p className="text-xs text-gray-500">{c.party} • {c.disputedRole}</p>
        ...
        {view === ViewState.DETAIL && selectedCandidate && (
          <DetailView candidate={selectedCandidate} onBack={handleBack} />
        )}
        ...
        {view === ViewState.METHODOLOGY && (
          <div className="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-sm border border-gray-200 animate-fade-in">
             ...
             <div className="prose prose-blue text-gray-700">
               <p className="mb-4">
                 A metodologia "Ficha de Eficiência" cruza dados de 12 fontes oficiais para criar um índice comparável entre candidatos de diferentes esferas.
               </p>
               ...
               <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 flex gap-3 mt-8">
                 ...
                 <div>
                   ...
                   <p className="text-sm text-yellow-800">
                     Este é um projeto demonstrativo de tecnologia cívica. Os dados aqui apresentados ("Candidato A", etc) podem conter simulações para fins de demonstração da interface. Sempre verifique informações oficiais no site do TSE antes de votar.
                   </p>
                 </div>
               </div>
             </div>
